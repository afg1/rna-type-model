...
SO_GRAPH = obonet.read_obo("./utils/so-simple.obo")

BASE_SO_TERMS = ["SO:0000655", "SO:0000188", "SO:0000836", "SO:0000673"]

NCBI_GRAPH = obonet.read_obo("./utils/ncbitaxon.obo")
...
def lowest_common_ancestor(node1, node2) -> str:
    ...
    lengths = {len(nx.shortest_path(NCBI_GRAPH, p, "NCBITaxon:1")): p for p in common_path}
...
def same_domain(accession:str, model:str) ->bool:
    ...
    for node in common_path:
        if "NCBITaxon:superkingdom" in NCBI_GRAPH.nodes[node].get('property_value', [""])[0]:
            return True
    return False
