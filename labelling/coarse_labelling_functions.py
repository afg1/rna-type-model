...
from utils.ontology_tests import is_child_of, same_domain, specificity, lowest_common_ancestor
import numpy as np
...
def length_based(x):
    ...
    """
    if all([l < 100 for l in x['sequence_stop'] ]) and all([l > 50 for l in x['sequence_stop']]):
        return RNATypeCoarse.tRNA
    else:
        return RNATypeCoarse.abstain
...
def taxonomy_based_r2dt(x):
    ...
    domain_similarity = [same_domain(acc_tax, mod_tax) for acc_tax, mod_tax in zip(x['ac_taxid'], x['model_taxid']) ]
